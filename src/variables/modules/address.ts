...
export const middleSchool = () => {
  const prefixes = [
    ...
    '实验', '师范', '民办', '私立', '公立', '重点', '高级', '初级', '联合',
    '红旗', '解放', '建设', '人民', '光明', '胜利', '新华', '朝阳', '希望',
    '育才', '英才', '博雅', '华师', '师大', '理工', '科技', '外语', '艺术'
  ]
  
  const middleSchoolTypes = ['中学', '初级中学', '高级中学', '实验中学', '外国语学校', '附属中学', '第一中学', '高级中学', '初级中学']
  
  const schoolType = sample(middleSchoolTypes)
  const prefix = sample(prefixes)
  
  // 添加地区名称使学校更真实
  const locationPart = Math.random() > 0.5 ? county() : city()
  const cleanLocation = locationPart.replace(/区|县|市|旗/g, '')
  
  if (Math.random() > 0.5) {
    return `${cleanLocation}${prefix}${schoolType}`
  } else {
    return `${cleanLocation}${schoolType}`
  }
}
